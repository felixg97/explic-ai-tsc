...
LIME proposed by Ribeiro et al. 
...
class LimeBase():
    ...
    def generate_lars_path(weighted_data, weighted_labels):
        ...
        alphas, _, coefs = lars_path(x_vector, weighted_labels, method='lasso', 
            verbose=False)
    ...
    def feature_selection(self, data, labels, weights, num_features, method):
        ...
        if method == 'none':
            ...
            if sp.sparse.issparse(data):
                ...
                # most efficient to slice the data before reversing
                sdata = len(weighted_data.data)
                argsort_data = np.abs(weighted_data.data).argsort()

                # Pad with zero-valued features, when the data is more sparse
                # than requested number of features
                if sdata < num_features:
                    nnz_indices = argsort_data[::-1]
                    indices = weighted_data.indices[nnz_indices]
                    ...
                    nnz_indices = argsort_data[sdata - num_features:sdata][::-1]
                    indices = weighted_data.indices[nnz_indices]
                return indices
            else:
    ...
    def explain_instance_with_data(self, neighborhood_data, neighborhood_labels,
        distances, label, num_features, feature_selection='auto', model_regressor=None):
        """Takes perturbed data, labels and distnaces, return explanation.
        Args:
